...
"""
from render_utils import maybe_linked, render_list_section, render_base_page_template, render_table_content
...
def render_row(data, data_dir):
    ...
    if data.get('author'):
        ...
        author_html = f'<p><strong>Author:</strong> {maybe_linked("mathematicians", author_short, data_dir=data_dir)}</p>'
    # Make the whole card clickable, linking to the row page
    row_link = f"{data['short_name']}.html" if data.get('short_name') else None
    if row_link:
        title_html = f'<h3><a href="{row_link}" class="table-title-link">{data["name"]}</a></h3>'
    else:
        title_html = f'<h3>{data["name"]}</h3>'
    html = f'''
    <div class="table-card" id="eq-{data['id']}">
        {title_html}
        ...
        <div class="table-details">
            ...
            {render_list_section(data.get('applications', []), 'Applications') if data.get('applications') else ''}
        </div>
    </div>
    '''
    return html
...
def render_row_page(row, schema, data_dir):
    ...
    back_link = '<div class="back-link-light"><a href="index.html">&larr; Back to Equations Table</a></div>'
